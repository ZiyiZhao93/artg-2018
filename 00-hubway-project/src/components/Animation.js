...
function Animation(_){
	...
	function exports(trips, stations){
		...
		//take DOM element, trips, stations
		...
		stationData = stations.map(stn => {
			const [x,y] = projection([stn.lng, stn.lat]); //xy is [a,b]
			return {
				x,
				y,
				...stn //
		...
		const stationsEnter = stationsNodes.enter()
			.append('g')
			.attr('class', 'station');
		stationsEnter.append('circle').attr('r', 10)
			.style('fill', 'none')
			.style('stroke', 'rgba(255,255,255,.3)')
			.style('stroke-width', 1);
		stationsEnter.append('text').text(d => d.id_short).style('font-size', '10');
		stationsEnter
			.merge(stationsNodes)
