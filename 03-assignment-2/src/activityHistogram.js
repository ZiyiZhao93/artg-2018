...
export default function activityHistogram(data, i){
	...
	const svg = d3.select(this)
		.selectAll('svg')
		.data([1]); //What's going on here? -- only one element
	...
	const tripsByQuarterHour = histogram(data.values) //has to be array, can not be an object
		.map(d => {
			return {
